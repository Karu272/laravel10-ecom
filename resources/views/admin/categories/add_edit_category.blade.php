...
@section('content')
    <div class="col-sm-6">
        ...
        <form name="categoryForm" id="categoryForm"
            @if (empty($editCat['id'])) action="{{ route('admin.categories.add_edit_category') }}"
      @else
            ...
            <div class="form-row">
                ...
                <div class="form-group col-md-6">
                    ...
                    <select name="parent_id" class="form-control">
                        ...
                        <option class="oSizeT" value="0" @if($editCat['parent_id'] == 0 ) selected="" @endif>&lceil;Main Category&rceil;</option>
                        @foreach ($getCategories as $cat)
                            <option @if(isset($editCat['parent_id']) && $editCat['parent_id'] == $cat['id']) selected  @endif value="{{ $cat['id'] }}">&loz; {{ $cat['category_name'] }} &loz;</option>
                            @if (!empty($cat['subcategories']))
                                @foreach ($cat['subcategories'] as $subcat)
                                    <option @if(isset($editCat['parent_id']) && $editCat['parent_id'] == $subcat['id']) selected  @endif value="{{ $subcat['id'] }}">&nbsp;&nbsp;&raquo; {{ $subcat['category_name'] }}
                                    </option>
                                    @if (!empty($subcat['subcategories']))
                                        @foreach ($subcat['subcategories'] as $subsubcat)
                                            <option value="{{ $subsubcat['id'] }}">
                                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&rarr; {{ $subsubcat['category_name'] }}
                                            </option>
                                        @endforeach
                                    @endif
                                @endforeach
